...
st.markdown("""
<style>
  [data-testid="stAppViewContainer"] { background-color:#080c14; color:#e2e8f0; }
  [data-testid="stSidebar"]          { background-color:#0d1320; border-right:1px solid #1e293b; }
  [data-testid="stSidebar"] * { color:#e2e8f0 !important; }
  h1,h2,h3 { color:#38bdf8 !important; }
  div[data-testid="metric-container"] {
    background:#0d1320; border:1px solid #1e293b; border-radius:12px; padding:16px;
  }
  .viol-card { background:rgba(248,113,113,0.08); border-left:4px solid #f87171; border-radius:8px; padding:12px 16px; margin-bottom:8px; }
  .highlight-card { background:rgba(34,197,94,0.08); border-left:4px solid #22c55e; border-radius:8px; padding:12px 16px; margin-bottom:8px; }
</style>
""", unsafe_allow_html=True)


# ══════════════════════════════════════════════════════
# LOAD DATA — works locally AND on cloud
# ══════════════════════════════════════════════════════
@st.cache_data(ttl=30)
def load_all_results() -> list:
    results = []

    # Load from local analysis_results/ folder
...
with st.sidebar:
    ...
    if not results:
        st.warning(
            ...
            "Run the app and analyze some calls first."
        )
        st.markdown("### 📤 Upload Results JSON")
        uploaded = st.file_uploader(
            "Upload scored_*.json files",
            type    = ["json"],
...
with col2:
    ...
    fig2.update_layout(
        plot_bgcolor="#0d1320", paper_bgcolor="#0d1320",
        font_color="#e2e8f0", margin=dict(l=0,r=0,t=0,b=0),
...
with col4:
    ...
    fig4.update_layout(
        plot_bgcolor="#0d1320", paper_bgcolor="#0d1320",
        font_color="#e2e8f0", margin=dict(l=0,r=0,t=0,b=0),
        height=280, legend=dict(orientation="v",font=dict(color="#e2e8f0"))
    )
...
fig5.add_trace(go.Scatter(
    ...
    marker = dict(color=[score_color(s) for s in df_f["score"]], size=8),
...
fig5.update_layout(
    plot_bgcolor="#0d1320", paper_bgcolor="#0d1320",
    font_color="#e2e8f0", margin=dict(l=0,r=0,t=20,b=0),
    height=260, xaxis_title="Call #",
...
with tab1:
    ...
    for _, row in worst.iterrows():
        ...
        st.markdown(
            ...
            f"Issue: {str(row['issue'])[:80]}</div></div>",
            unsafe_allow_html=True
        )
